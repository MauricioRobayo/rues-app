...
export async function generateMetadata({
  ...
  const { nit } = parseCompanyPathSegment(company);
  const companyData = await getCompanyData(nit);
  // TODO: I should parse this data before getting here
  const formattedNit = formatNit(Number(companyData.rues.nit));
  return {
    title: `${companyData.rues.razon_social} - NIT ${formattedNit}`,
...
export const getCompanyData = unstable_cache(
  cache(async (nit: number) => {
    ...
    // TODO: properly map and return the massaged data
    return {
      ...companyData,
      siis,
    };
